...
struct AudioWaveVertex
{
    v3f position = {};
    f32 distanceFromCenter = 0.f;
};
...
#define AUDIO_WAVE_SAMPLES 512

struct AudioWaveRenderable
{
    Handle<Mesh> h_Mesh;
    Handle<ShaderPipeline> h_Shader;
    Handle<Material> h_Material;
    ...
    // Renderable Uniforms
    m4f u_Model = Identity();
    f32 u_AudioSamples[AUDIO_WAVE_SAMPLES];

    void PropagateWave(f32 nextSample);
    void SetWaveData(f32* sampleData);
};
...
Handle<Mesh>            Renderer_CreateMesh(Handle<Buffer> h_VertexBuffer, Handle<Buffer> h_IndexBuffer, bool audioWave = false);
...
struct RendererData
{
    ...
    AudioWaveRenderable audioWaveRenderable;
};
...
inline AudioWaveRenderable& Renderer_GetAudioWaveRenderable() { return rendererData.audioWaveRenderable; }
