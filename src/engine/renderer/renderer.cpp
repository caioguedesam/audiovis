...
Handle<Mesh> Renderer_CreateMesh(Handle<Buffer> h_VertexBuffer, Handle<Buffer> h_IndexBuffer, bool audiowave)
{
    APIHandle glHandle = API_HANDLE_INVALID;
    ...
    {
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(AudioWaveVertex), (void*)StructOffset(AudioWaveVertex, position));
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(AudioWaveVertex), (void*)StructOffset(AudioWaveVertex, distanceFromCenter));
        glEnableVertexAttribArray(1);
    }
        // Audio wave specific (this definitely needs more flexibility)
        Mesh mesh =
    {
        h_VertexBuffer,
        h_IndexBuffer,
...
Handle<Shader> Renderer_CreateShader(std::string_view shaderSrc, ShaderType shaderType)
{
    ...
    {
        char infoLog[1024];
        glGetShaderInfoLog(glHandle, GL_COMPILE_STATUS, NULL, infoLog);
        ASSERT(0);
    }
...
Handle<ShaderPipeline> Renderer_CreateShaderPipeline(Handle<Shader> h_VS, Handle<Shader> h_PS)
{
    ...
    {
        char infoLog[1024];
        glGetProgramInfoLog(glHandle, GL_LINK_STATUS, NULL, infoLog);
        ASSERT(0);
    }
...
void AudioWaveRenderable::PropagateWave(f32 nextSample)
{
    for(i32 i = AUDIO_WAVE_SAMPLES - 1; i > 0; i--)
    {
        u_AudioSamples[i] = u_AudioSamples[i-1];
    }
    u_AudioSamples[0] = Lerp(u_AudioSamples[1], nextSample, 0.2f);
}

void AudioWaveRenderable::SetWaveData(f32* sampleData)
{
    memcpy(u_AudioSamples, sampleData, sizeof(f32) * AUDIO_WAVE_SAMPLES);
}
...
void Renderer_RenderFrame()
{
    ...
    RenderTarget& renderTargetDefault = Renderer_GetRenderTarget(h_RenderTarget_Default);
    {
        ...
        Renderer_Clear({0.f, 0.f, 0.f, 1.f});
...
//    }
    ...
    // Rendering audio wave mesh
    {
        PROFILE_GPU_SCOPED("Render Audio Mesh");
        AudioWaveRenderable& waveRenderable = rendererData.audioWaveRenderable;
        Renderer_BindShaderPipeline(waveRenderable.h_Shader);
        //Renderer_BindMaterial(waveRenderable.h_Material);
        Renderer_BindMesh(waveRenderable.h_Mesh);

        // Uniforms
        ShaderPipeline& shader = Renderer_GetShaderPipeline(waveRenderable.h_Shader);
        GLint location = -1;
        location = glGetUniformLocation(shader.apiHandle, "u_Model");
        ASSERT(location != -1);
        glUniformMatrix4fv(location, 1, GL_TRUE, &waveRenderable.u_Model.m00);
        for(i32 i = 0; i < AUDIO_WAVE_SAMPLES; i++)
        {
            char buf[AUDIO_WAVE_SAMPLES];
            sprintf(buf, "u_AudioSamples[%d]", i);
            location = glGetUniformLocation(shader.apiHandle, buf);
            ASSERT(location != -1);
            glUniform1f(location, waveRenderable.u_AudioSamples[i]);
        }
        ...
        Mesh& mesh = Renderer_GetMesh(waveRenderable.h_Mesh);
        Buffer& indexBuffer = Renderer_GetBuffer(mesh.h_IndexBuffer);
        glDrawElements(GL_TRIANGLES, indexBuffer.count, GL_UNSIGNED_INT, 0);
